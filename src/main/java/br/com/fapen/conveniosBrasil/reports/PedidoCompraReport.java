package br.com.fapen.conveniosBrasil.reports;
...
public class PedidoCompraReport {
	...
	public ByteArrayInputStream gerarPDF(PedidoCompra pedidoSendoImpresso) {
		...
		try {
			...
			String logo = resourceLoader.getResource("/WEB-INF/templateEmail/conveniosbrasil.png").getFile().getAbsolutePath();
			...
			for (int cont = 0; cont < pedidoSendoImpresso.getItens().size(); cont++) {
				...
				tabelaItens.addCell(new Paragraph(itemPed.getQuantidade().toString()).addStyle(normal)
						.setTextAlignment(TextAlignment.CENTER));
				tabelaItens.addCell(new Paragraph("R$ " + itemPed.getPrecoUnitario().toString()).addStyle(normal)
						.setTextAlignment(TextAlignment.CENTER));
				tabelaItens.addCell(new Paragraph("R$ " + itemPed.getPrecoTotal().toString()).addStyle(normal)
						.setTextAlignment(TextAlignment.CENTER));
			}
			tabelaItens.addCell(
					new Cell(1, 4).add(new Paragraph("Total").addStyle(normal).setTextAlignment(TextAlignment.RIGHT)));
			tabelaItens.addCell(new Cell(1, 1).add(new Paragraph("R$ " + pedidoSendoImpresso.getValorTotal().toString())
					.addStyle(negrito).setTextAlignment(TextAlignment.CENTER)));
	...
	private void defineFontes() throws IOException {
		...
		this.titulo = new Style().setFont(fontCorpo)
				.setFontSize(16).setFontColor(ColorConstants.BLACK)
				.setTextAlignment(TextAlignment.CENTER)
				.setBold()
				.setPaddingTop(15);
	}
