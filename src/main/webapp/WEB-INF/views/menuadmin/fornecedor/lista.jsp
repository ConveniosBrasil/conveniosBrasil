				...
				<div class="row">
					...
					<f:form method="GET">
						<div class="input-field col s5">
							<input id="busca" name="busca" id="search" type="text"
								placeholder="Pesquisar por Nome Fantasia" value="${busca}">
						</div>
					...
					<div class="input-field col s3">
						<a
							...
							title="novo" href="${ s:mvcUrl('novoFornecedorUrl').build() }">
										...
										<sec:authorize access="hasRole('ROLE_ADMIN')">
											<td>
												<c:if test="${registro.visivel == 'S'}">
													<i class="material-icons iconsRegistroAtivo" title="Fornecedor ativo">check_circle</i>
												</c:if>
												<c:if test="${registro.visivel == 'N'}">
													<i class="material-icons iconsRegistroInativo" title="Fornecedor inativo">cancel</i>
												</c:if>
											</td>
										</sec:authorize>
										<td class="center-align tabelaColunaAcoes"><a class="btn-small green"
											title="alterar" href="${s:mvcUrl('alterarFornecedorUrl').arg(0, registro.id).build()}"><i
												class="material-icons icons-white">edit</i></a> <a
											class="btn-small" title="detalhes"
											href="${s:mvcUrl('visualizarFornecedorUrl').arg(0, registro.id).build()}"><i
												class="material-icons icons-white">menu_book</i></a>
											<sec:authorize access="hasRole('ROLE_ADMIN')">	
												<c:if test="${registro.visivel == 'N'}">
													<button class="btn-small green btnAtivar modal-excluir" title="excluir"
														type="button" data-descr="${ registro.razaoSocial }" data-visivel="${ registro.visivel }">Ativar
														<i class="material-icons right">offline_pin</i>
														<f:form
															action="${s:mvcUrl('ativarFornecedorUrl').arg(0, registro.id).build()}"
															method="POST"></f:form>
													</button>
												</c:if>
												<c:if test="${registro.visivel == 'S'}">
													<button class="btn-small red modal-excluir" title="excluir"
														type="button" data-descr="${ registro.razaoSocial }" data-visivel="${ registro.visivel }">Inativar
														<i class="material-icons right">delete</i>
														<f:form
															action="${s:mvcUrl('inativarFornecedorUrl').arg(0, registro.id).build()}"
															method="POST"></f:form>
													</button>
												</c:if>	
											</sec:authorize>
											<sec:authorize access="!hasRole('ROLE_ADMIN')">
												<button class="btn-small red modal-excluir" title="excluir"
													...
													data-descr="${ registro.razaoSocial }">
